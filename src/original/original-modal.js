...
export class Modal {
  ...
  async alert(message, {okButton, ...attributes} = {}) {
    okButton = {
  ...
  async confirm(message, {okButton, cancelButton, ...attributes} = {}) {
    okButton = {
      title: this.getOKButtonTitle(),
      value: true,
      default: true,
          ...
          if (attrs.render) {
            ...
            if (attrs.buttons && attrs.buttons.length) {
              ...
              for (let i = 0; i < attrs.buttons.length; i++) {
                ...
                props.onClick = () => {
                  if (button.onClick) {
                    button.onClick({close: attrs._onClose});
                  } else {
                    attrs._onClose(button.value);
                  }
                };
